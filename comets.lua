...
function comet:new()
	...
	while self.dx == 0 do self.dx = self[math.random(#self)] end
	while self.dy == 0 do self.dy = self[math.random(#self)] end
...
function comet:update(dt)
	...
	or self.y < -love.graphics.getHeight() or self.y > love.graphics.getHeight() * 2 then
		self.e = nil
	end
end
